...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const npm = new npm_1.NPMClient(cfg.npm.url);
            yield npm.login(cfg.npm.email, cfg.npm.password);
            core.info("Retrieved authentication token from npm");
            core.endGroup();
            core.startGroup('Create or Renew SSL Certificate');
            let certificate = yield npm.getCertificateByDomain(cfg.ssl.domain);
            if (certificate) {
                core.info("SSL Certificate exist, renew");
                yield npm.renewCertificate(certificate.id);
            }
            else {
                core.info("SSL Certificate isn't exist, crete new");
                certificate = yield npm.createCertificate(cfg.ssl.domain, cfg.ssl.email);
            }
            core.endGroup();
            core.startGroup('Create or Update Host');
            let host = yield npm.getHostByDomain(cfg.host.domain);
            if (host) {
                core.info("Host exist, update");
                yield npm.updateHost(host.id, cfg.host, certificate.id);
            }
            else {
                core.info("Host isn't exist, crete new");
                certificate = yield npm.createHost(cfg.host, certificate.id);
            }
            core.endGroup();
        }
        catch (e) {
...
class NPMClient {
    ...
    login(email, password) {
        return __awaiter(this, void 0, void 0, function* () {
            const { data } = yield this.client.post('/api/tokens', {
                identity: email,
                secret: password
            });
    ...
    getCertificates() {
        return __awaiter(this, void 0, void 0, function* () {
            const { data } = yield this.client.get('/api/nginx/certificates', {
                params: {
                    expand: 'owner'
                }
            });
    ...
    getCertificateByDomain(domain) {
        return __awaiter(this, void 0, void 0, function* () {
            const certificates = yield this.getCertificates();
            return certificates.find((c) => c.nice_name === domain);
        });
    }
    renewCertificate(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const { data } = yield this.client.post(`/api/nginx/certificates/${id}/renew`);
            return data || [];
        });
    }
    createCertificate(domain, email) {
        return __awaiter(this, void 0, void 0, function* () {
            const { data } = yield this.client.post(`/api/nginx/certificates`, {
                domain_names: [
                    domain
                ],
                meta: {
    ...
    getHosts() {
        return __awaiter(this, void 0, void 0, function* () {
            const { data } = yield this.client.get('/api/nginx/proxy-hosts', {
                params: {
    ...
    getHostByDomain(domain) {
        return __awaiter(this, void 0, void 0, function* () {
            const hosts = yield this.getHosts();
            return hosts.find((h) => h.domain_names.includes(domain));
        });
    }
    createHost(host, certificate_id) {
        return __awaiter(this, void 0, void 0, function* () {
            const { data } = yield this.client.post(`/api/nginx/proxy-hosts`, {
                domain_names: [
                    host.domain
                ],
    ...
    updateHost(id, host, certificate_id) {
        return __awaiter(this, void 0, void 0, function* () {
            const { data } = yield this.client.put(`/api/nginx/proxy-hosts/${id}`, {
                domain_names: [
                    host.domain
                ],
                forward_scheme: host.schema,
